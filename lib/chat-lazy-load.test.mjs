...
test("reattach tolerance falls back to the desktop window on non-coarse environments", async () => {
  const { getReattachTolerance, CHAT_SCROLL_REATTACH_TOLERANCE, CHAT_SCROLL_MOBILE_REATTACH_TOLERANCE } = await loadSubject();
  // In a headless/node test there is no `window`, so it must not throw and
  // should return the desktop reattach window.
  assert.equal(getReattachTolerance(), CHAT_SCROLL_REATTACH_TOLERANCE);
  assert.equal(CHAT_SCROLL_MOBILE_REATTACH_TOLERANCE, 40);
});
