...
test("'.fnos.net' suffix-collision attacks are rejected (no dot before fnos)", async () => {
  const { isApiRequestHostAllowed } = await loadSubject();
  // Explicit pattern (no default catch-all) to isolate .fnos.net suffix behavior.
  const patterns = [".fnos.net"];
  for (const host of [
    "evilfnos.net:30141",       // no dot before fnos -- classic suffix-collision
    "xevilfnos.net:30141",      // same shape
    "notfnos.net:30141",        // another collision
    "fnos.net.attacker.com",    // attack domain that contains fnos.net as label
  ]) {
    assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
      headers: { host },
    }), patterns), false, host);
...
test("'.fnos.net' suffix wildcard allows any legitimate subdomain shape", async () => {
  const { isApiRequestHostAllowed } = await loadSubject();
  const patterns = [".fnos.net"];
  for (const host of [
    "fnos.net:30141",              // apex
    "appvmm.fnos.net:30141",       // single-label
    "a.b.c.fnos.net:30141",        // multi-level
    "evil.fnos.net:30141",         // "attack-shaped" but IS a legitimate subdomain
    "123.fnos.net:30141",          // numeric label
    "abc-xyz-123.fnos.net:30141",  // dynamic / random subdomain
  ]) {
    assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
      headers: { host },
    }), patterns), true, host);
...
test("'.fnos.net' rejects unrelated apex domains", async () => {
  const { isApiRequestHostAllowed } = await loadSubject();
  const patterns = [".fnos.net"];
  for (const host of [
    "evil.com:30141",
    "attacker.org:30141",
    "fnosnet.com:30141",         // looks like fnos.net but is a different apex
    "fnos.com:30141",            // different TLD
  ]) {
    assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
      headers: { host },
    }), patterns), false, host);
...
test("mixed patterns: exact + '.fnos.net' wildcard work together", async () => {
  const { isApiRequestHostAllowed } = await loadSubject();
  const patterns = ["pi-web.internal", "192.168.9.109", ".fnos.net"];
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "pi-web.internal:30141" },
  }), patterns), true);
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "192.168.9.109:30141" },
  }), patterns), true);
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "foo.fnos.net:30141" },
  }), patterns), true);
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "evil.com:30141" },
  }), patterns), false);
});

test("'*.fnos.net' (with *.) also rejects suffix-collision attacks", async () => {
  const { isApiRequestHostAllowed } = await loadSubject();
  const patterns = ["*.fnos.net"];
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "evilfnos.net:30141" },
  }), patterns), false);
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "notfnos.net:30141" },
  }), patterns), false);
  // Note: *.fnos.net explicitly does NOT match the apex "fnos.net" itself
  // (matches subdomains only). This is the documented semantic for *. prefix.
  assert.equal(isApiRequestHostAllowed(new Request("http://localhost/api/test", {
    headers: { host: "fnos.net:30141" },
  }), patterns), false);
});
