...
test("keeps live following cancellable when the user scrolls away from the tail", () => {
  ...
  assert.match(scrollHandlerSource, /const isAgentRunning = agentRunningRef\.current;[\s\S]*?isAgentRunning\s*\? getReattachTolerance\(\)\s*:\s*CHAT_SCROLL_TAIL_TOLERANCE/);
  assert.match(source, /previousScrollTopRef\.current = scrollTop/);
  assert.match(scrollToBottomSource, /if \(!container\) return;[\s\S]*?container\.scrollTo\(\{ top: container\.scrollHeight, behavior }\);[\s\S]*?previousScrollTopRef\.current = container\.scrollTop/);
