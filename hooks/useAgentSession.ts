...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleExtensionUiRequest = useCallback((request: ExtensionUiRequest) => {
    switch (request.method) {
      ...
      case "input":
      case "editor":
        // === notify: emit inputNeeded event ===
        {
          const summary = request.method === "select"
            ? `${request.title} (select from ${request.options.length} options)`
          ...
          emitNotifyEvent({
            ...
            summary,
            detail: request.method === "confirm" ? request.message : undefined,
          });
        }
  ...
  }, [addNotice, cancelEventStreamGrace, handleExtensionUiRequest, loadSession, messages, notifyPromptStage, onAgentEnd, scheduleEventStreamClose, session, settleUiStage]);
